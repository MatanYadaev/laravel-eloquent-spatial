...
it('creates a model record with line string with SRID', function (): void {
  ...
  expect($testPlace->line_string->srid)->toBe(SRID::GCS->value);
});
...
it('creates line string with SRID from JSON', function (): void {
  ...
  $lineStringFromJson = LineString::fromJson('{"type":"LineString","coordinates":[[180,0],[179,1]]}', SRID::GCS->value);
...
it('creates line string with SRID from WKT', function (): void {
  ...
  $lineStringFromWkt = LineString::fromWkt('LINESTRING(180 0, 179 1)', SRID::GCS->value);
